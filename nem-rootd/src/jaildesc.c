...
static const struct {
	NEM_jailimg_isolate_t iso;
	const char *name;
}
jailimg_isolate_names[] = {
	{ NEM_ISOLATE_JAIL,     "jail"     },
	{ NEM_ISOLATE_CAPSICUM, "capsicum" },
	{ NEM_ISOLATE_NETWORK,  "network"  },
};

static const NEM_jailimg_isolate_t
...
jailimg_isolate_nonames[] = {
	{ NEM_ISOLATE_JAIL,     "no_jail"        },
	{ NEM_ISOLATE_CAPSICUM, "no_capsicum"    },
	{ NEM_ISOLATE_NETWORK,  "expose_network" },
};
...
static const struct {
	NEM_jailimg_type_t type;
	const char *name;
}
jailimg_types[] = {
	{ NEM_IMG_INVALID,    "invalid" },
	{ NEM_IMG_SINGLEFILE, "file"    },
	{ NEM_IMG_IMAGE,      "image"   },
	{ NEM_IMG_VNODE,      "vnode"   },
	{ NEM_IMG_SHARED,     "shared"  },
};

static NEM_jailimg_type_t
jailimg_type_from_name(const char *name)
{
	...
	return NEM_IMG_INVALID;
}

#define TYPE NEM_jailimg_t
static const NEM_marshal_field_t NEM_jailimg_fs[] = {
...
MAP(NEM_jailimg_m, NEM_jailimg_fs);
#undef TYPE

NEM_err_t
NEM_jailimg_valid(const NEM_jailimg_t *this)
{
	NEM_jailimg_type_t type = jailimg_type_from_name(this->type);

	if (NEM_IMG_INVALID == type) {
		return NEM_err_static("NEM_jailimg_valid: invalid type");
	}
	if (NULL == this->dest || 0 == this->dest[0]) {
		return NEM_err_static("NEM_jailimg_valid: missing or empty dest");
	}
	if ('/' != this->dest[0]) {
		return NEM_err_static("NEM_jailimg_valid: doesn't start with a /");
	}

	if (NULL == this->name || 0 == this->name[0]) {
		if (NEM_IMG_VNODE == type) {
			if (this->persist) {
				return NEM_err_static(
					"NEM_jailimg_valid: persisted vnode must be named"
				);
			}
			// okay.
		}
			...
			return NEM_err_static(
				"NEM_jailimg_valid: non-vnode image must be named"
			);
		}
	}
	...
	if (NEM_IMG_VNODE == type) {
		if (0 == this->len) {
			return NEM_err_static(
				"NEM_jailimg_valid: non-vnode image must have non-zero "
			    "size"
			);
		}
	}

	if (NEM_IMG_SHARED == type || NEM_IMG_VNODE == type) {
		if (NULL != this->semver || NULL != this->sha256) {
			return NEM_err_static(
				"NEM_jailimg_valid: vnode/shared images cannot be "
				"versioned"
			);
		}
	}
...
NEM_jailimg_type_t 
NEM_jailimg_type(const NEM_jailimg_t *this)
{
	return jailimg_type_from_name(this->type);
}

#define TYPE NEM_jaildesc_t
static const NEM_marshal_field_t  NEM_jaildesc_fs[] = {
...
MAP(NEM_jaildesc_m, NEM_jaildesc_fs);
#undef TYPE

bool
NEM_jaildesc_use_isolate(
	const NEM_jaildesc_t *this,
	NEM_jailimg_isolate_t iso
) {
	int this_iso = NEM_jaildesc_isolate(this);
	return this_iso & iso;
}

int
NEM_jaildesc_isolate(const NEM_jaildesc_t *this)
{
	...
	for (size_t i = 0; i < this->isolation_flags_len; i += 1) {
		...
		NEM_panicf("NEM_jaildesc_isolate: invalid flag %s", flag);
	}

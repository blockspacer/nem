...
typedef struct NEM_md_t {
	RB_ENTRY(NEM_md_t) link;
...
NEM_mdlist_add_provider(NEM_mdlist_t *this, const struct gprovider *prov)
{
	...
	LIST_FOREACH(cons, &prov->lg_consumers, lg_consumers) {
		...
		else if (!strcmp("VFS", cons->lg_geom->lg_class->lg_name)) {
			num_vfs += 1;
		}
		else if (!strcmp("PART", cons->lg_geom->lg_class->lg_name)) {
			num_part += 1;
		}
	}
	if (0 != num_part) {
